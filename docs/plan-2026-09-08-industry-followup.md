...
> 创建日期: 2026-09-08
> 类型: 技术跟进路线图（不是架构重构）
> 与 `plan.md` 的关系: **不挂钩**。`plan.md` 管架构债（状态所有权 / broadcast 拆分 /
>   channels 收敛），本计划管「外部世界变了，nanobot 要不要跟」。两条线并行，互不阻塞。
...
- 批次 D 的业界数据（92%→58%）来自别人的生产系统，nanobot 的数字可能完全不同——
  所以第 2 步是自己测，不是照搬结论。
- 本计划全程不碰 `plan.md` 的 Phase 1/2/3 范围。唯一交叠点是批次 B 要往 `events.py`
  加两个事件——那是新增，不改现有状态流。

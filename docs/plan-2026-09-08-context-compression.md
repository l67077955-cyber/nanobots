...
> 与其他计划的关系:
>   - 根 `plan.md`（状态所有权 / broadcast 拆分 / channels 收敛）——**架构线，本计划不碰**。
>     其 Phase 1 step 3（`flip_running` 退役 + `broadcast_round` 返回 `session_should_stop`）
>     仍是那条线的下一个 checkpoint，截至本计划创建**未动手**。
...
| 与 plan.md 架构线（Phase 1 step 3）并行冲突 | 中 | 中 | 两条线改动面不重叠（本计划不碰 runtime 状态机）；均需全量测试绿才提交 |

## 不做（本轮范围外）

- `plan.md` Phase 1/2/3 的任何条目（那条线自己推进）

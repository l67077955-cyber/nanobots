    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        if action == "disable":
            ...
            if not hasattr(self._engine, "_session_tools_override"):
                self._engine._session_tools_override = {}
            if agent not in self._engine._session_tools_override:
...
class ClearContextTool(Tool):
    ...
    async def _clear_one(self, agent: str, keep_last: int, reason: str) -> str:
        ...
        total = history.count_by_attr("agent", agent)
        ...
        removed = history.delete_by_attr("agent", agent, keep_last=keep_last)

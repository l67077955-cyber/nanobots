        ...
        try:
            while True:
                ...
                if cycle == 1 and not content and not (set(result.tools_used or []) & _substantive_tools):
                    ...
                    messages.append({
                        "role": "system",
                        "content": engine.prompt_builder.get_system_warning("idle", name=name)
                    })
                ...
                # Force a summary cycle so the output is not silently swallowed.
                    ...
                    messages.append({
                        "role": "system",
                        "content": engine.prompt_builder.get_system_warning("no_text_after_tools", name=name)
                    })
                ...
                # tool names, so the leader silently exits without a synthesis message.
                    ...
                    messages.append({
                        "role": "system",
                        "content": engine.prompt_builder.get_system_warning("leader_no_text_after_tools", name=name)
                    })
                    continue  # re-enter tool_loop to produce synthesis text

                # ── 关键修复3：Leader end_discussion 但没有文字输出时强制合成 ──
                if is_leader and _leader_ended_discussion and not content:
                    logger.warning(
                    ...
                    messages.append({
                        "role": "system",
                        "content": engine.prompt_builder.get_system_warning("leader_end_without_text", name=name)
                    })
                    continue  # 重新生成最终答案
                ...
                if msg is None:
                    ...
                    if is_leader and not content:
                        ...
                        messages.append({
                            "role": "system",
                            "content": engine.prompt_builder.get_system_warning("leader_wait_timeout", name=name)
                        })

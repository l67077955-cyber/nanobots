    ...
    async def _run_one(
        ...
        try:
            while True:
                ...
                # ── Pre-tool_loop pruning: prevent unbounded message growth ──
                # The pruning after wait() (line ~1381) only fires on the wait
                # path.  All continue-paths (timeout recovery, error recovery,
                # guard skips) bypass it, causing messages to grow without bound
                # and eventually trigger LLM timeouts.

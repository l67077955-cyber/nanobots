...
from nanobot.webui.providers_models_store import (
    providers_models_items,
    resolve_provider_from_store,
    upsert_provider_models_entry,
)
...
def _config_provider_entry(config: Any, provider_key: str) -> ProviderConfig | None:
    spec = find_by_name(provider_key)
    if spec is not None:
        provider_config = getattr(config.providers, spec.name, None)
        return provider_config if isinstance(provider_config, ProviderConfig) else None
    normalized = provider_key.replace("-", "_")
    for extra_name, provider_config in _dynamic_provider_items(config):
        if provider_key == extra_name or normalized == extra_name.replace("-", "_"):
...
def _resolve_settings_provider(
    ...
    resolved_store = resolve_provider_from_store(provider_name)
    if resolved_store is not None:
        store_name, provider_config = resolved_store
        return create_dynamic_spec(store_name), store_name, provider_config
    return None
...
def settings_payload(
    *,
    ...
    for provider_key, provider_config in providers_models_items():
        if provider_key in known_provider_names:
            for index, row in enumerate(providers):
                if row["name"] != provider_key:
                    continue
                if not row["configured"] and _provider_configured_for_settings(
                    create_dynamic_spec(provider_key),
                    provider_config,
                ):
                    providers[index] = _provider_settings_row(
                        provider_key,
                        create_dynamic_spec(provider_key),
                        provider_config,
                    )
...
def update_provider_settings(query: QueryParams) -> dict[str, Any]:
    ...
    if changed:
        if _config_provider_entry(config, provider_key) is not None:
            save_config(config)
        upsert_provider_models_entry(provider_key, provider_config)
    image_config = config.tools.image_generation
    restart_required = (
        changed

...
class CallbacksMixin:
    ...
    async def _on_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            elif data.startswith("ef:") or data.startswith("ef_re:") or data.startswith("tf:") or data.startswith("srr:") or data.startswith("log:") or data.startswith("log_pg:") or data.startswith("rlog_pg:") or data.startswith("rlogs_pg:") or data.startswith("rlogp:") or data.startswith("rlog_dl:") or data.startswith("rlog:") or data.startswith("rlogctx:") or data.startswith("logd:") or data.startswith("logp:"):
                await self._handle_edit_and_logs(query, data, chat_id)
            ...
            elif data == "pm_cancel" or data == "st_prov" or data == "st_agent" or data.startswith("pm_newm:") or data.startswith("pm_delp:") or data.startswith("pm_delp_yes:") or data.startswith("pm_delm_p:") or data.startswith("pm_delm:") or data.startswith("em_prov:") or data.startswith("em_mi:") or data.startswith("em_model:") or data.startswith("em_manual:") or data.startswith("ep_pick:") or data.startswith("ep_field:") or data.startswith("ep_retry:") or data.startswith("ep_retry_set:") or data.startswith("ep_models:") or data.startswith("ml_pfx:") or data.startswith("ml_srch:") or data.startswith("ep_addm:"):
                await self._handle_provider_models(query, data, chat_id)
    ...
    async def _handle_provider_models(self, query, data: str, chat_id: str) -> None:
        """Handle provider/model management callbacks (pm/em/ep/ml/srr)."""
        if data == "pm_cancel":
            self._edit_state.pop(chat_id, None)
            await query.edit_message_text("❌ 已取消")
    ...
    async def _handle_edit_and_logs(self, query, data: str, chat_id: str) -> None:
        """Handle agent-edit (ef/ef_re/tf/srr) and log-viewer callbacks."""

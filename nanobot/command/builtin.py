...
BUILTIN_COMMAND_SPECS: tuple[BuiltinCommandSpec, ...] = (
    ...
    BuiltinCommandSpec(
        "/restart",
        "Restart nanobot",
        "Restart the bot process in background (closes attached CLI/frontend).",
        "rotate-cw",
    ),
    BuiltinCommandSpec(
        "/status",
...
async def cmd_restart(ctx: CommandContext) -> OutboundMessage:
    """Restart: use background spawn + exit current so any attached CLI/frontend terminal is closed."""
    msg = ctx.msg

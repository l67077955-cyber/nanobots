...
| 2026-09-13 | 外线（非本计划施工线）运维修复上线：pm 路由劫持（裸名被 gateway 前缀抢走→402 断网级故障）、DeepSeek 官方四条消息校验（content/reasoning_content/孤儿 tool_calls 双向）、直连路由 per-attempt 超时 20s→90s（61s 假超时根因）、startup billboard。终态 887 passed。**新增 backlog（源自 12:48 数字果蝇网页轮复盘）**：① 大工具结果（如 22KB web_fetch）入共享视图前先老化/摘要——Benjamin 视图膨胀到 30K chars 且前缀缓存命中率仅 3-33%，是超时恶性循环主因；② 恢复重试（broadcast.py:906 无工具 600token 短调用）对写码类任务零产能，考虑带 1-2 次工具机会；③ glm-5.3（TTFT 21-28s）不宜承担长输出任务，编排层可按模型 TTFT 分派；④ 并发写同一文件竞态——Harper 的 "/tmp 构建+原子替换" 值得升格为 write_file 工具约定。 |